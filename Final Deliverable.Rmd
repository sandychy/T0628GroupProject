...
```{r}
base_model <- glm(data_train_labels ~ ., data = data_train, family = "binomial")
summary(base_model)
```
...
```{r}
churn_binary_pred <- ifelse(data_pred >= 0.5, 1, 0)
...
```{r}
library(caret)
confusionMatrix(as.factor(churn_binary_pred), as.factor(data_test_labels), positive = "1")
```
