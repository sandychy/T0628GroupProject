...
- ANN -> Accuracy : 77.44%, Kappa: 0.4041,
- Decision Tree -> Accuracy : 78.75%, Kappa : 0.3937
- Random Forest -> Accuracy : 79.6%, Kappa : 0.4405
- **Second-level Decision Tree -> Accuracy : 80.4%, Kappa : 0.5108**
...
library(class)
ksize<-round(sqrt(nrow(data_train)),0) 
knn_base_model <- knn(data_train, data_test, data_train_labels, k = 173)
```
...
# rf_high_model <- train(as.factor(Churn) ~ ., data=data_t_train, method = "rf",
           metric = "Kappa",
           trControl = ctrl,
           tuneGrid = grid)
...
- Accuracy: 79.15% (simple model: 78.69%)
- Kappa: 0.4226 (simple model: 0.4223)