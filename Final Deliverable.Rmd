...
#CrossTable(x = as.factor(data_test_labels), y = rf_pred, 
#          prop.chisq=FALSE)
```
...
For the cost, we assigned 2 for false negatives and 1 for false positives.

```{r}
error_cost = matrix(c(0,1,2,0), nrow=2) # Cost - FN=8, FP=1. TP and TN don't have costs, so 0.
error_cost
...
```
As a result, the number of false negatives was decreased to 31 (was 46) while the number of false positives increased to 35 (was 27).

The accuracy and the kappa increased to 81.25% and 0.5472 respectively.
